...
@Component({
	...
	template: `
    <div *ngFor="let project of projects" id="project">
  ...
  `,
	styles: [
		'#project { width: 70%; margin: auto; margin-bottom: 150px; position: relative; background-color: #6f56ff; border-radius: 10px; }',
		'#project-title { margin: 0; position: absolute; top: 15%; left: 10%; right: 50%; color: white; }',
		'#project-description { margin: 0; position: absolute; top: 40%; left: 10%; right: 60%; color: white; }',
		'#project-image { margin-left: 50%; width: 50%; border-radius: 10px; }',
		'#project-links {margin: 0; bottom: 15%; left: 10%; position: absolute; }',
		'#project-highlights { top: 93%; margin-left: 5%; position: absolute; width: 100%; }',
